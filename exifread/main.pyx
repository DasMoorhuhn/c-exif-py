...
cdef tuple _find_tiff_exif(fh: BinaryIO) -> tuple:
    logger.debug("TIFF format recognized in data[0:2]")
    fh.seek(0)
    cdef int endian = fh.read(1)
    fh.read(1)
    cdef int offset = 0
    cdef tuple offset_endian = (offset, endian)
    return offset_endian


cdef tuple _find_webp_exif(fh: BinaryIO) -> tuple:
    ...
    if data[0:4] == b'VP8X' and data[4] & 8:
        ...
        while True:
            ...
            if data[0:4] == b'EXIF':
                fh.seek(6, 1)
                cdef tuple offset_and_endian = (fh.tell(), fh.read(1))  # (offset, endian)
                return offset_and_endian
            cdef int size = struct.unpack('<L', data[4:8])[0]
            fh.seek(size, 1)
    raise ExifNotFound("Webp file does not have exif data.")


cdef _find_png_exif(fh: BinaryIO, data) -> tuple:
    logger.debug("PNG format recognized in data[0:8]=%s", data[:8].hex())
    fh.seek(8)

    while True:
        cdef int data = fh.read(8)
        cdef int chunk = data[4:8]
        ...
        cdef int chunk_size = int.from_bytes(data[:4], "big")
        fh.seek(fh.tell() + chunk_size + 4)

    raise ExifNotFound("PNG file does not have exif data.")


cdef bytes _get_xmp(fh: BinaryIO) -> bytes:
    cdef bytes xmp_bytes = b''
    logger.debug('XMP not in Exif, searching file for XMP info...')
    cdef bool xml_started = False
    cdef bool xml_finished = False
    for line in fh:
        cdef int open_tag = line.find(b'<x:xmpmeta')
        cdef int close_tag = line.find(b'</x:xmpmeta>')
        if open_tag != -1:
            xml_started = True
            line = line[open_tag:]
...
cdef tuple _determine_type(fh: BinaryIO) -> tuple:
    # by default do not fake an EXIF beginning
    cdef int fake_exif = 0

    cdef bytes data = fh.read(12)
    if data[0:2] in [b'II', b'MM']:
        # it's a TIFF file
        cdef int offset, endian = _find_tiff_exif(fh)
    elif data[4:12] == b'ftypheic':
        fh.seek(0)
        heic = HEICExifFinder(fh)
        cdef int offset, endian = heic.find_exif()
    elif data[0:4] == b'RIFF' and data[8:12] == b'WEBP':
        cdef int offset, endian = _find_webp_exif(fh)
    elif data[0:2] == b'\xFF\xD8':
        # it's a JPEG file
        cdef int offset, endian, fake_exif = find_jpeg_exif(fh, data, fake_exif)
    elif data[0:8] == b'\x89PNG\r\n\x1a\n':
        cdef int offset, endian = _find_png_exif(fh, data)
    else:
        raise ExifNotFound("File format not recognized.")
    cdef tuple offset_endian_fake_exif = (offset, endian, fake_exif)
    return offset_endian_fake_exif


cpdef process_file(fh: BinaryIO, stop_tag=DEFAULT_STOP_TAG,
    ...
    try:
        cdef int offset, endian, fake_exif = _determine_type(fh)
    ...
    cdef str endian = chr(ord_(endian[0]))
    ...
    cdef hdr = ExifHeader(fh, endian, offset, fake_exif, strict, debug, details, truncate_tags)
    ifd_list = hdr.list_ifd()
    cdef int thumb_ifd = 0
    cdef int ctr = 0
    ...
    for ifd in ifd_list:
        ...
        ctr += 1
    # EXIF IFD
    cdef exif_off = hdr.tags.get('Image ExifOffset')
    if exif_off:
    ...
    if debug and details:
        ...
        cdef xmp_tag = hdr.tags.get('Image ApplicationNotes')
        if xmp_tag:
            logger.debug('XMP present in Exif')
            cdef xmp_bytes = bytes(xmp_tag.values)
